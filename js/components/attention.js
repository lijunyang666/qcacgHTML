...
var attention = Vue.extend({
      ...
      +'<div v-for="obj in bookList.attention"><img class="img-attention-head" :src="path.rootPath + obj.userHead" /><div class="attention-name"><span>{{ obj.userName }}</span><span class="attention-name-sex">♂</span><div class="wordAI ">+关注</div></div><span class="wordAI-two" v-if="obj.information">{{ obj.information }}</span><span class="wordAI-two" v-else>这个人很赖,没有留下简介</span><hr style="border: 1px solid #E1E1E1; margin:20px 0 0 5%;" width="90%" />'
      +'</div></div>'
