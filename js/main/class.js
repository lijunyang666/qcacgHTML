	...
	var vuePractice = new _vueMin2.default({
	  ...
	  data: {
	    ...
	    obj: {
	      ...
	      sort: [], // 分类
	      fontsize: [{ name: '全部', min: 0, max: '', id: 0 }, { name: '10万字以下', min: 0, max: 100000, id: 1 }, { name: '10-30万字', min: 100000, max: 300000, id: 2 }, { name: '50-100万字', min: 500000, max: 1000000, id: 3 }, { name: '一百万字以上', min: 1000000, max: "", id: 4 }], // 字数
	  ...
	  methods: {
	    ...
	    setHengshu: function setHengshu(v) {
	      // 横竖切换显示
	    ...
	    getValueFn: function getValueFn() {
	      ...
	      this.$set('bookInfo', []);
	      _vueHttp2.default.http(this, 'get', _conf2.default.queryBookClass, this.RequestObj, function (response) {
	        _this.bookInfo = response.data.bookInfo;
	        if (!response.data.bookInfo) {
	          _this.$set('bookInfo', []);
	          _this.$set('verticalBoookInfo', []);
	          _this.setPageCount(1);
	          return;
	        }
	        var arr = [];
	        if (response.data.bookInfo.length % 4 === 0) {
	          for (var i = 1; i <= response.data.bookInfo.length; i++) {
	            ...
	            if (i % 4 === 0) {
	              _this.verticalBoookInfo.push(arr);
	              arr = [];
	            }
	          }
	        } else {
	    ...
	    sortFn: function sortFn(id) {
	      this.RequestObj.bookTypeId = id;
	      this.getValueFn(); // 请求
	    },
	    newbookFn: function newbookFn(id) {
	      // 排序方法
	      this.RequestObj.sort = id;
	      this.getValueFn();
	    },
	    fontsizeFn: function fontsizeFn(min, max, id) {
	      // 请求字数区间的
	      this.fontsizeId = id;
	    ...
	    qianyueFn: function qianyueFn(id) {
	      // 签约与否的
	  ...
	  ready: function ready() {
	    ...
	    if (_vueHttp2.default.getQueryString('id')) {
	      this.RequestObj.bookTypeId = _vueHttp2.default.getQueryString('id');
	    }
